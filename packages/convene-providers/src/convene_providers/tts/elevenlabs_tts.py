...
class ElevenLabsTTS(TTSProvider):
    ...
    async def synthesize(self, text: str) -> AsyncIterator[bytes]:
        """Synthesize text into streaming audio bytes via ElevenLabs.

        Sends a POST request to ElevenLabs' text-to-speech streaming
        endpoint and yields audio chunks as they arrive.

        Args:
            text: The text to synthesize into speech.

        Yields:
            Audio bytes in MP3 format from ElevenLabs.
        """
